...
delta_rr_deploy = np.array([0, 0, 0.100])  # initial delta position = 10 m along +R-bar (towards Earth)
...
delta_vv_deploy = np.array([-0.007, 0, 0])  # initial delta position = 0.1 m/s along -V-bar (ortogonal to orbit)
...
a_cubesat, e_cubesat, i_cubesat, Omega_cubesat, omega_cubesat, theta_cubesat = rv2kp(rr0_cubesat, vv0_cubesat, mu_earth)
print('')
print('DECOMMISSIONING:')
print('Keplerian parameter:     {0}'.format(a_cubesat))
print('Pericenter:              {0} km'.format(a_cubesat * (1 - e_cubesat)))
print('Apocenter:               {0} km'.format(a_cubesat * (1 + e_cubesat)))
